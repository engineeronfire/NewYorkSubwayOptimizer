...
def task1(mtaTable, mtaUpdateObj):
    while(1):
        ...
        for update in updates:
            ...
            print 'saving'
     #       if update.routeId == '1' or update.routeId == '2' or update.routeId == '3':
     #           print update.futureStops
               # write(update.tripId, update.routeId, update.startDate, convertTime(int (time.time())), "weekday", "day")
        time.sleep(30)

def write(tripId, routeId, startDate, time, day, date):
    with open('trip.cvs', 'a') as file:
        file.write('%s,%s,%s,%s,%s,%s\n' % (tripId, routeId, startDate, time, day, date))
